...
if [ `id -u` -eq 0 ]; then
   function root_menulist() { 
      ...
      # fi
      if [ $root_highlight -eq 1 ] ; then
         echo "${bldgreen}1. Create a USER and /home/USER -- done!${txtrst}"
      else
         echo "1. Create a USER and /home/USER"
      fi
      if [ $root_highlight -eq 2 ] ; then
         echo "${bldgreen}2. Edit /etc/pacman.conf, install git+yaourt, create locale/timezone/hostname -- done!${txtrst}"
      else
         echo "2. Edit /etc/pacman.conf, install git+yaourt, create locale/timezone/hostname"
      fi
      ...
      if [ $root_highlight -eq 4 ] ; then
         echo "${bldgreen}4. Switch to your newly created USER and Finalize install -- done!${txtrst}" 
      else
         echo "4. Switch to your newly created USER and Finalize install" 
      fi
...
sudo cp ${my_home}${dotfiles}etc/custom.conf /etc/X11/xorg.conf.d/custom.conf
   "
         ...
         mkdir -p .moc
         cp ${my_home}${dotfiles}moc.config ${my_home}.moc/config
   ...
   cp ${my_home}${dotfiles}.excludes-crypt ${my_home}.excludes-crypt${txtrst}"
         ...
         sudo cp ${my_home}${dotfiles}etc/autologin\@.service /etc/systemd/system/autologin\@.service
